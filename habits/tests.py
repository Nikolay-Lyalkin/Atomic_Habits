...
class HabitTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.habit = Habit.objects.create(
            user=self.user, action="Вечерняя прогулка", time="20:00:00", award="Фруктовая тарелка"
        )

    def test_habit_create(self):
        data = {"action": "Утренняя растяжка", "time": "08:00:00", "award": "Пирожное Rex"}
...
class PleasantHabitTestCase(APITestCase):
    def setUp(self):
        ...
        self.pleasant_habit = PleasantHabit.objects.create(
            user=self.user, action="Поход в Grammy", place="Grammy", sign_pleasant_habit="True"
        )
    ...
    def test_validator_pleasant_habit_create(self):
        ...
        self.assertEquals(
            response.json(),
            {"non_field_errors": ["Могут быть добавлены привычки только с признаком приятной привычки"]},
        )

    def test_pleasant_habit_create(self):
        data = {"action": "Утренний кофе", "place": "Шоколадница", "sign_pleasant_habit": "True"}
